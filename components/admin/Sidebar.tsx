            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={
                    `block px-4 py-3 mx-2 rounded-lg text-sm font-medium transition-all duration-200 min-h-[44px] min-w-[44px] ` +
                    (active
                      ? "bg-white text-[#c0392b] border-l-4 border-[#c0392b] font-bold shadow-sm"
    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-[#111827] flex items-center px-4 z-50">
        <button
          ...
          className="text-white mr-4 focus:outline-none focus:ring-2 focus:ring-[#c0392b] min-h-[44px] min-w-[44px]"
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 h-full bg-[#111827] shadow-xl animate-slide-in-left">
            <button
              onClick={() => setOpen(false)}
              className="absolute top-4 right-4 text-gray-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-[#c0392b] min-h-[44px] min-w-[44px]"
